...
export class EnvVariableComponent implements OnInit {
  ...
  listColumns: any[] = [
    ...
    { title: this.i18n.transform('base_var_value')},
  ];

  constructor(private envVariablesManagement: EnvironmentVariablesManagementService,
  ...
  public async getEnvVariables(): Promise<void> {
    try {
      this.loading = true;
      const result =
        await this.envVariablesManagement.getEnvVariablesDetail(this.envId);
