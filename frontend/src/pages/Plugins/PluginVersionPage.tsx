  ...
  useEffect(() => {
    ...
    if (versionDataResponse?.code === 200 && versionDataResponse.data) {
      ...
      const pluginData: Plugin = {
        ...
        category: versionData.plugin_info.plugin_type === 1 ? '云侧插件' : '本地插件',
        status: 'active',
      ...
      console.error('获取插件版本信息失败:', versionError)
      showError('获取插件版本信息失败，请稍后重试')
    } else if (versionDataResponse && versionDataResponse.code !== 200) {
      showError(`获取插件版本信息失败: ${versionDataResponse.message}`)
    }
  }, [versionDataResponse, versionLoading, versionError, plugin_id, version])

  const handleSaveConfig = useCallback(async () => {
    if (isReadOnly) {
      showError('当前为只读模式，不允许编辑插件配置')
      return
    }
    ...
    try {
      ...
      console.error('保存插件版本配置失败:', error)
      const errorMessage = error?.response?.data?.message || error?.message || '保存插件版本配置失败，请稍后重试'
  ...
  const handlePluginSubmit = useCallback(
    async (isEditing: boolean = false) => {
      ...
      if (isEditing && editingPlugin) {
        ...
        showSuccess(`插件版本"${updatedPlugin.name}"更新成功`)
  ...
  const handlePublishPlugin = useCallback(
    async (version: string, versionDesc: string) => {
      ...
      try {
        ...
        if (response.code === 200) {
          showSuccess(`插件版本"${plugin?.name || plugin_id}"发布成功！`)
        ...
        console.error('发布插件版本失败:', error)
        const errorMessage = error?.response?.data?.message || error?.message || '发布插件版本失败，请稍后重试'
        showError(errorMessage)
      }
    },
