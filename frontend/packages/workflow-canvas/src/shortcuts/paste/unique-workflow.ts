...
namespace UniqueWorkflowUtils {
  ...
  export const generateNodeReplaceMap = (nodeIds: string[], isUniqueId: (id: string) => boolean): Map<string, string> => {
    ...
    nodeIds.forEach(id => {
      if (isUniqueId(id)) {
        ...
        let newId: string
        do {
          // 保留原始ID的前缀部分（最后一个下划线之前的部分），只替换UUID部分
          const lastUnderscoreIndex = id.lastIndexOf('_')
          const prefix = lastUnderscoreIndex !== -1 ? id.substring(0, lastUnderscoreIndex + 1) : ''
          newId = prefix + customNanoid(5) // generate new id with prefix until unique - 生成带前缀的新ID直到唯一
  ...
  const shouldHandle = (context: TraverseContext): boolean => {
    ...
    if (node?.key && ['sourceNodeID', 'targetNodeID'].includes(node.key) && node.parent?.parent?.key === 'edges') {
      return true
    }
    // check node data - 检查节点数据
    if (node?.key === 'id' && isExist(node.container?.type) && isExist(node.container?.meta) && isExist(node.container?.data)) {
      return true
    }
    // check variable data - 检查变量数据
    if (node?.key === 'blockID' && isExist(node.container?.name) && node.container?.source === 'block-output') {
      return true
    }
    return false
  }
   ...
   */
  export const replaceNodeId = (json: WorkflowJSON, nodeReplaceMap: Map<string, string>): WorkflowJSON => {
...
export const generateUniqueWorkflow = (params: { json: WorkflowJSON; isUniqueId: (id: string) => boolean }): WorkflowJSON => {
