...
export class DeleteShortcut implements ShortcutsHandler {
  ...
  public async execute(nodes?: WorkflowNodeEntity[]): Promise<void> {
    ...
    if (!this.isValid(selection.filter(n => n instanceof WorkflowNodeEntity) as WorkflowNodeEntity[])) {
      return
    }
  ...
  private isValid(nodes: WorkflowNodeEntity[]): boolean {
    const hasSystemNodes = nodes.some(n => [WorkflowNodeType.Start, WorkflowNodeType.End].includes(n.flowNodeType as WorkflowNodeType))
    if (hasSystemNodes) {
      Toast.error({
