...
import type { WorkflowClipboardRect, WorkflowClipboardSource, WorkflowClipboardData } from '../type'
...
export class CopyShortcut implements ShortcutsHandler {
  ...
  private get selectedNodes(): WorkflowNodeEntity[] {
    return this.selectService.selection.filter(n => n instanceof WorkflowNodeEntity) as WorkflowNodeEntity[]
  }
  ...
  private getValidNodes(nodes: WorkflowNodeEntity[]): WorkflowNodeEntity[] {
    return nodes.filter(n => {
      if ([WorkflowNodeType.Start, WorkflowNodeType.End].includes(n.flowNodeType as WorkflowNodeType)) {
        return false
      }
  ...
  private getNodeJSONs(nodes: WorkflowNodeEntity[]): WorkflowNodeJSON[] {
    const nodeJSONs = nodes.map(node => (node.flowNodeType === FlowNodeBaseType.GROUP ? this.getGroupNodeJSON(node) : this.document.toNodeJSON(node)))
  ...
  private getEdgeJSONs(nodes: WorkflowNodeEntity[]): WorkflowEdgeJSON[] {
    ...
    expandedNodes.forEach(node => {
      ...
      lines.forEach(line => {
        if (line.from?.id && nodeIdSet.has(line.from.id) && line.to?.id && nodeIdSet.has(line.to.id)) {
          lineSet.add(line)
        }
      })
    })
  ...
  private notifySuccess(): void {
    const startEndNodeTypes: WorkflowNodeType[] = [WorkflowNodeType.Start, WorkflowNodeType.End, WorkflowNodeType.BlockStart, WorkflowNodeType.BlockEnd]
    if (this.selectedNodes.some(node => startEndNodeTypes.includes(node.flowNodeType as WorkflowNodeType))) {
      Toast.warning({
        content: 'The Start/End node cannot be duplicated, other nodes have been copied to the clipboard',
        showClose: false,
      })
      return
    }
