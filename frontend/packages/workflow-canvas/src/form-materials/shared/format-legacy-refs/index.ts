 ...
 *
 * Example:
 * import { formatLegacyRefOnSubmit, formatLegacyRefOnInit } from '../../';
 * formMeta: {
...
export function formatLegacyRefOnSubmit(value: any): any {
  if (isObject(value)) {
    ...
    return Object.fromEntries(Object.entries(value).map(([key, value]: [string, any]) => [key, formatLegacyRefOnSubmit(value)]))
  }
...
export function formatLegacyRefOnInit(value: any): any {
  if (isObject(value)) {
    ...
    return Object.fromEntries(Object.entries(value).map(([key, value]: [string, any]) => [key, formatLegacyRefOnInit(value)]))
  }
...
export function isLegacyFlowRefValueSchema(value: any): value is LegacyFlowRefValueSchema {
  return isObject(value) && Object.keys(value).length === 2 && (value as any).type === 'ref' && typeof (value as any).content === 'string'
}

export function isNewFlowRefValueSchema(value: any): value is NewFlowRefValueSchema {
  return isObject(value) && Object.keys(value).length === 2 && (value as any).type === 'ref' && Array.isArray((value as any).content)
}
