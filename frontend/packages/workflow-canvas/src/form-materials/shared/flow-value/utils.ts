...
export namespace FlowValueUtils {
  ...
  export function* traverse(
    ...
    const { includeTypes = ['ref', 'template', 'expression', 'constant'], path = '', pathArr = [] } = options || {}
  ...
  export function getTemplateKeyPaths(value: IFlowTemplateValue) {
    ...
    const keyPathReg = /\{\{([^\}\{]+)\}\}/g
    return uniq(value.content?.match(keyPathReg) || []).map(_keyPath => _keyPath.slice(2, -2).split('.'))
  }
