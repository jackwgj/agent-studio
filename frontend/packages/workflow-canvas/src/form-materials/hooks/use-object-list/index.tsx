...
export function useObjectList<ValueType>({
      ...
      onChange(
        Object.fromEntries(
          nextList
            ...
            .map((_res, idx) => {
              const indexKey = typeof sortIndexKey === 'function' ? sortIndexKey(_res[1] as ValueType | undefined) : sortIndexKey
  ...
  const updateKey = (itemId: string, key: string) => {
    ...
    setList(prevList => {
      ...
      onChange(Object.fromEntries(nextList.filter(item => item.key).map(item => [item.key!, item.value])))
