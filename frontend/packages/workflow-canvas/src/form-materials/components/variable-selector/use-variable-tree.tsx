...
import { IJsonSchema, JsonSchemaTypeManager, JsonSchemaUtils, useTypeManager } from '@flowgram.ai/json-schema'
...
export function useVariableTree(params: {
  ...
  const renderVariable = (variable: VariableField, parentFields: VariableField[] = []): TreeNodeData | null => {
  ...
  return [...variables.slice(0).reverse()].map(_variable => renderVariable(_variable as VariableField)).filter(Boolean) as TreeNodeData[]
}
