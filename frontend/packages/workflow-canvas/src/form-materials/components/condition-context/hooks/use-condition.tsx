...
export function useCondition({ leftSchema, operator, onClearOp, onClearRight, ruleConfig }: HooksParams) {
  const typeManager = useTypeManager()
  const { rules: contextRules, ops: contextOps } = useConditionContext()

  // Merge user rules and context rules
  const userRules = useMemo(() => ruleConfig?.rules || contextRules || {}, [contextRules, ruleConfig?.rules])

  // Merge user operators and context operators
  const allOps = useMemo(() => ruleConfig?.ops || contextOps || {}, [contextOps, ruleConfig?.ops])

  // Get type configuration
  const config = useMemo(() => (leftSchema ? typeManager.getTypeBySchema(leftSchema) : undefined), [leftSchema, typeManager])
