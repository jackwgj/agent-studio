...
import { BaseVariableField, Disposable, DisposableCollection, Scope, useCurrentScope } from '@flowgram.ai/editor'
import { Popover, Tag } from '@douyinfe/semi-ui'
import { IconIssueStroked } from '@douyinfe/semi-icons'
import { Decoration, DecorationSet, EditorView, MatchDecorator, ViewPlugin, WidgetType } from '@codemirror/view'

import { IPolyfillRoot, polyfillCreateRoot } from '../../../'
...
class VariableTagWidget extends WidgetType {
  ...
  renderVariable(v?: BaseVariableField) {
    if (!v) {
      this.root.render(
        <Tag className="gedit-m-base-editor-tag" color="amber">
    ...
    const rootTitle = <span className="gedit-m-base-editor-root-title">{rootField.meta?.title ? `${rootField.meta.title} ${isRoot ? '' : '-'} ` : ''}</span>
    const rootIcon = this.renderIcon(rootField?.meta.icon)

    this.root.render(
      <Popover
        content={
          <div className="gedit-m-base-editor-popover-content">
            {rootIcon}
            {rootTitle}
            <span className="gedit-m-base-editor-var-name">{v?.keyPath.slice(1).join('.')}</span>
          </div>
        }
      >
        <Tag className="gedit-m-base-editor-tag" style={{ display: 'inline-flex', alignItems: 'center' }}>
          {rootIcon}
          {rootTitle}
          {!isRoot && <span className="gedit-m-base-editor-var-name">{v?.key}</span>}
        </Tag>
      </Popover>,
    )
  }
  ...
  toDOM(view: EditorView): HTMLElement {
    ...
    this.toDispose.push(this.scope.available.trackByKeyPath(this.keyPath, refresh, { triggerOnInit: false }))
