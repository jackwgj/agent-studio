...
import { DataEvent, Effect, EffectOptions, VariableFieldKeyRenameService } from '@flowgram.ai/editor'
          ...
          if (_v.type === 'ref') {
            ...
            templateKeyPaths.forEach(_keyPath => {
              if (isKeyPathMatch(_keyPath, beforeKeyPath)) {
                hasMatch = true
                const nextKeyPath = [...afterKeyPath, ...(_keyPath || [])?.slice(beforeKeyPath.length)]
                _v.content = _v.content?.replace(`{{${_keyPath.join('.')}}`, `{{${nextKeyPath.join('.')}}`)
              }
            })
 ...
 * @returns
 */
function traverseRef(name: string, value: any, cb: (name: string, _v: IFlowRefValue | IFlowTemplateValue) => void) {
