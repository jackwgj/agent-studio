...
export class AuthService {
  ...
  // 用户注册（不需要密码）
  static async register(userData: { username: string; password?: string; grant_type?: string }): Promise<LoginResponse> {
    const apiClient = getApiClient()
    // 将对象转换为URL编码的字符串
    const formData = new URLSearchParams()
  ...
  static async validateToken(): Promise<ApiResponse<{ valid: boolean; expiresAt?: string }>> {
    const apiClient = getApiClient()
    const response = await apiClient.get<ApiResponse<{ valid: boolean; expiresAt?: string }>>(API_ENDPOINTS.USERS.DETAIL)
    return response.data
  }
  ...
  static async validateTokenSafely(): Promise<ApiResponse<{ valid: boolean; expiresAt?: string }>> {
    ...
    const response = await tempApiClient.get<ApiResponse<{ valid: boolean; expiresAt?: string }>>(API_ENDPOINTS.USERS.DETAIL)
    return response.data
  }
