...
@Service("Agent_RelateResourceCleaner")
public class AgentRelationCleaner implements IRelateSourceCleaner {
        ...
        if (ids.size() > 0) {
            ...
            for (String id : ids) {
                if (!StringUtils.isEmpty(id)) {
                    obsService.deleteByPrefix(String.format("agent/flow/%s/", id));
                    obsService.deleteByPrefix(String.format("agent/dsl/%s/", id));
                    obsService.deleteByPrefix(String.format("agent/ir/%s/", id));
                }
            }
