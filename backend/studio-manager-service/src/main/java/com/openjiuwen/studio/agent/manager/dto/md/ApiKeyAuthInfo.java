...
package com.openjiuwen.studio.agent.manager.dto.md;
...
public class ApiKeyAuthInfo implements ProviderAuth {
    ...
    public String convertToMaskStr(boolean decrypt) {
        if (decrypt) {
            return StringUtils.join("{\"API Key\":\"", AgentStringUtils.mask(CryptoUtils.decrypt(apiKey), 4), "\"}");
        } else {
            return StringUtils.join("{\"API Key\":\"", AgentStringUtils.mask(apiKey, 4), "\"}");
        }
    }
}
