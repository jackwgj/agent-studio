 ...
 */
@Slf4j
public class ReferenceHandler extends BaseTypeHandler<Reference> implements
    AttributeConverter<Reference, String> {
    private final ObjectMapper objectMapper = new ObjectMapper();
    @Override
    public void setNonNullParameter(PreparedStatement ps, int i, Reference parameter, JdbcType jdbcType)
        throws SQLException {
        ps.setString(i, JSONObject.toJSONString(parameter));
    }

    @Override
    public Reference getNullableResult(ResultSet rs, String columnName) throws SQLException {
        String result = rs.getString(columnName);
        if (result != null) {
            return JSONObject.parseObject(result, Reference.class);
        }
        return null;
    }

    @Override
    public Reference getNullableResult(ResultSet rs, int columnIndex) throws SQLException {
    ...
    @Override
    public Reference getNullableResult(CallableStatement cs, int columnIndex) throws SQLException {
    ...
    public Reference convertToEntityAttribute(String dbData) {
        try {
            ...
            return objectMapper.readValue(dbData, Reference.class);
        } catch (IOException e) {
