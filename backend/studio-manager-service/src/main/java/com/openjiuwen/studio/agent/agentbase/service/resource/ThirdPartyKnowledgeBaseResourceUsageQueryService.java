...
@Slf4j
@Service
public class ThirdPartyKnowledgeBaseResourceUsageQueryService implements ResourceUsageQueryService {

    private final KnowledgeBaseMapper knowledgeBaseMapper;

    public ThirdPartyKnowledgeBaseResourceUsageQueryService(KnowledgeBaseMapper knowledgeBaseMapper) {
        this.knowledgeBaseMapper = knowledgeBaseMapper;
    }

    @Override
    public ResourceType resourceType() {
        return ResourceType.THIRD_PARTY_KNOWLEDGE_BASE;
    }

    @Override
    public Long queryResourceUsageCount(String domainId) {
        return (long) this.knowledgeBaseMapper.countByType(KnowledgeBaseType.EXTERNAL.getValue(), domainId).size();
    }
