...
@Slf4j
@Service
public class SIESkillManagementService implements ISkillManagementService {
    ...
    public SIESkillManagementService(SIESkillMapper sieskillMapper, SkillVersionMapper sKillVersionMapper) {
    ...
    public Void deleteStudioSkill(String skillId, String workspaceId, String projectId) {
        // 越权校验
        SkillDetails skill = sieskillMapper.searchBySkillIdAndDomainId(skillId, RequestContextUtils.getRequestUserDomainId());
        if (skill == null) {
    ...
    public ImportStudioSkillResponseBody importStudioSkill(String workspaceId, String projectId, MultipartFile multipartFile) {
        ...
        // 构造Skill实体并保存（publishedAsset=1 使导入的skill同时显示在组件库和资产广场）
        SkillEntity skillEntity = new SkillEntity()
                ...
                .setPublishedAsset(1);
        sieskillMapper.insert(skillEntity);
    ...
    public ExportStudioSkillResponseBody exportStudioSkill(String skillId, String workspaceId,
        ...
        // 查询Skill基本信息
        SkillDetails skillEntity = sieskillMapper.searchBySkillId(skillId);
        if (skillEntity == null) {
    ...
    public ListStudioSkillsResponseBody listStudioSkills(String projectId, ListStudioSkillsQo listStudioSkillsQo) {
        ...
        List<SkillDetails> selectedSkills = sieskillMapper.search(selectCondition, listStudioSkillsQo.getOffset(),
            listStudioSkillsQo.getLimit(), listStudioSkillsQo.getPriorityStatus(),listStudioSkillsQo.getTagId(), publishedAsset);

        // 2. 构建响应体
        ListStudioSkillsResponseBody responseBody = new ListStudioSkillsResponseBody().setTotal(sieskillMapper.countSelectedSkills(selectCondition));
