...
from openjiuwen.core.component.end_comp import TemplateProcessor, TemplateBatchProcessor

from app.core.common.dsl import UserOutputConfig
...
class UserOutputComponent(ComponentExecutable, WorkflowComponent):
    ...
    async def _render(self, inputs: Input):
        ...
        self._batch_template = None
        await self.create_output_result(answer)
        return {
            "output": answer,
        }
    ...
    async def invoke(self, inputs: Input, runtime: Runtime, context: Any) -> Output:
        ...
        if not self.output_writer:
            self.output_writer = runtime.stream_writer()
        ...
        if output_message:
            ...
            result = inputs
        await self.create_output_result(result)
        return {
            "output": result
        }

    async def stream(self, inputs: Input, runtime: Runtime, context: Any) -> AsyncIterator[Output]:
        ...
        try:
            if self.template_processor is not None:
                ...
                async for frame in generator:
                    ...
                    result = await self.create_output_result(frame)
                    yield result
            else:
                if inputs is None:
                    return
                for key, value in inputs.items():
                    result = await self.create_output_result({key: value})
                    yield result
    ...
    async def transform(self, inputs: Input, runtime: Runtime, context: Any) -> AsyncIterator[Output]:
            ...
            for (path, value) in extract_leaf_nodes(inputs):
                if isinstance(value, AsyncGenerator):
                    async for frame in value:
                        result = await self.create_output_result({format_path(path): frame})
                        yield result
                else:
                    result = await self.create_output_result({format_path(path): value})
                    yield result

    async def collect(self, inputs: Input, runtime: Runtime, context: Any) -> Output:
        ...
        if self.template_processor is not None:
            ...
            await self.create_output_result(chunks)
            return {
                "output": chunks
            }

    async def create_output_result(self, output_content):
        """创建统一的输出结果格式"""
        ...
        await self.output_writer.write(output_schema)
