...
import com.openjiuwen.studio.agent.space.common.constant.SigningAlgorithm;
...
public class Signer {
    ...
    public void sign(SignRequest request) throws UnsupportedEncodingException {
    ...
    private String getCanonicalizedResourcePath(String resourcePath) {
        if (StringUtil.isNotEmptyString(resourcePath)) {
            try {
            ...
            String value = UrlUtil.urlEncode(resourcePath, true);
            if (!value.endsWith("/")) {
                value = value.concat("/");
            }
            if (!value.startsWith("/")) {
    ...
    protected String getCanonicalizedQueryString(Map<String, List<String>> parameters) throws UnsupportedEncodingException {
        ...
        for (Map.Entry<String, List<String>> entry : parameters.entrySet()) {
            String encodedParamName = UrlUtil.urlEncode(entry.getKey(), false);
            List<String> paramValues = entry.getValue();
            List<String> encodedValues = new ArrayList<>(paramValues.size());
            Collections.sort(paramValues);

            for (String value : paramValues) {
                encodedValues.add(UrlUtil.urlEncode(value, false));
            }
    ...
    private String buildAuthorizationHeader(String[] signedHeaders, byte[] signature, String accessKey) {
        ...
        String signerHeaders = "SignedHeaders=" + this.getSignedHeadersStr(signedHeaders);
        String signatureHeader = "Signature=" + StringUtil.toHex(signature);
    ...
    protected byte[] sign(byte[] data, byte[] key, SigningAlgorithm algorithm) {
        try {
            Mac signMac = Mac.getInstance(algorithm.toString());
            signMac.init(new SecretKeySpec(key, algorithm.toString()));
            return signMac.doFinal(data);
    ...
    protected String getCanonicalizedHeaderString(SignRequest request, String[] signedHeaders) {
        ...
        for (String header : signedHeaders) {
            ...
            String signHeaderStr = requestHeaders.get(header);
            buffer.append(key).append(":");
            if (signHeaderStr != null) {
                buffer.append(signHeaderStr.trim());
            }
    ...
    protected String getSignedHeadersStr(String[] signedHeaders) {
        StringBuilder buffer = new StringBuilder();
    ...
    public boolean verify(SignRequest request) throws UnsupportedEncodingException {
        String singerDate = this.getHeader(request, "X-Sdk-Date");
    ...
    protected String calculateContentHash(SignRequest request) {
        ...
        return Objects.requireNonNullElseGet(content_sha256,
                () -> StringUtils.equals(this.messageDigestAlgorithm, "SDK-HMAC-SHA256") ?
                        StringUtil.toHex(this.hash(request.getBody())) : StringUtil.toHex(this.hashSm3(request.getBody())));
    }
    ...
    public byte[] hash(String text) {
        try {
            ...
            log.error("Error while hashing text : {}", ex.getMessage());
            return new byte[0];
        }
    }
