...
public class RedisUtils {
    ...
    public boolean set(final String key, String value) {
        ...
        try {
            ...
            throw new AgentSpaceException("set cache failed");
        }
        return result;
    }
    ...
    public boolean set(final String key, String value, Long expireTime) {
        ...
        try {
            ...
            throw new AgentSpaceException("set cache with expireTime failed");
        }
        return result;
    }
    ...
    public long hmRemove(String key, String... hashKeys) {
        HashOperations<String, String, String> hash = redisTemplate.opsForHash();
        return hash.delete(key, (Object[]) hashKeys);
    }
        ...
        redisTemplate.executePipelined(new RedisCallback<List<Map<String, String>>>() {
            ...
            public List<Map<String, String>> doInRedis(RedisConnection connection) throws DataAccessException {
                ...
                while (iterator.hasNext()) {
                    ...
                    // 批量保存
                    redisTemplate.opsForHash().putAll(hashName, hashes);
                }
                return null;
            }
        });
    ...
    public void batchHashMapSet(String cacheName, Map<String, String> cacheMap) {
        HashMap<String, Map<String, String>> dataMap = new HashMap<>();
        dataMap.put(cacheName, cacheMap);
        batchHashMapSet(dataMap);
    }
    ...
    public void batchHashMapSet(Map<String, Map<String, String>> dataMap) {
        // 设置5秒超时时间, redisTemplate.expire("max", 25, TimeUnit.SECONDS);
        redisTemplate.executePipelined(new RedisCallback<List<Map<String, String>>>() {
            @Override
            public List<Map<String, String>> doInRedis(RedisConnection connection) throws DataAccessException {
                ...
                while (iterator.hasNext()) {
                    ...
                    Map<String, String> hashValues = hash.getValue();
                    Iterator<Map.Entry<String, String>> it = hashValues.entrySet().iterator();
                    // 使用 Map<String, String> 而不是 byte[] 来存储 Hash 内容
                    Map<String, String> hashes = new HashMap<>();
                    while (it.hasNext()) {
                        // hash中一条key-value记录
                        Map.Entry<String, String> entry = it.next();
                        // 直接将 entry 的 key 和 value 添加到 hashes 中
                        hashes.put(entry.getKey(), entry.getValue());
                    }
                    // 批量保存
                    redisTemplate.opsForHash().putAll(hash.getKey(), hashes);
                }
                return null;
            }
        });
     ...
     * 批量添加 并且设置失效时间
     *
     * @param cacheMap 缓存键值对
     * @param seconds  失效时间
     */
    public void batchSetOrExpire(Map<String, String> cacheMap, Long seconds) {
        RedisSerializer<String> serializer = redisTemplate.getStringSerializer();
        redisTemplate.executePipelined((RedisCallback<String>) connection -> {
            cacheMap.forEach((key, value) -> {
                connection.stringCommands()
                    .set(Objects.requireNonNull(serializer.serialize(key)),
                        Objects.requireNonNull(serializer.serialize(value)), Expiration.seconds(seconds),
