...
| 场景 | 优先查看 |
|------|----------|
| 运行中报错（执行阶段） | **Runtime** 日志 > Service 日志 |
| 未执行就报错（配置/定义阶段） | **Manager** 日志 |
| SSE 连接异常、超时 | **Service** 日志 |
...
| Service | `/opt/cloud/studio-service/logs/` | `studio-agent-service.log` | 由 `log4j2.xml` 配置，路径可通过 `LOG_PATH` 环境变量修改 |
