...
class Tool(Invokable):
    ...
    async def ainvoke(self, inputs: Input, **kwargs) -> Output:
        ...
        if self.provider_type == ProviderType.PLUGIN:
            if tool_name == self.invokable.name or self.invokable.name.startswith(
                ...
                aivoke_rsult = await self.invokable.ainvoke(
    ...
    async def _exec_workflow(self, inputs, **kwargs):
        """执行工作流"""
        err_func_res = dict(
            errMessage={
        ...
        try:
            res_generator = await self.invokable.astream(query, kwargs)
            ...
            logger.error(f"Panel execute workflow error {type(traceback_error_msg)}")
            err_msg = err_func_res.get("errMessage", {})
            err_msg.update(
                {
                    WORKFLOW_ANSWER: (
