...
from jiuwen.context.memory_engine.config.memory_ir_config import ENABLE_KEY
...
from jiuwen.prompt import Prompt, TemplateManager, Template
...
class IntentDetection(Invokable):
    ...
    def __init__(
        ...
        self.language_config = LanguageManager().get_language_context_for_node(
            self.language, "intention_detection"
        )
        ...
        if len(matching_items) > 0:
            # branch_0在里面，新意图默认分类0
            self.intent_config.default_class = self.language_config.get(
                "default_class_new"
            )
        else:
            # branch_0不在里面，新意图默认分类1
            self.intent_config.default_class = self.language_config.get(
                "default_class_old"
            )
        ...
        intent_prompt = TemplateManager().get(
            name=self.language_config.get("template_intention_detection"),
            filters={"model_name": model_configs.get(MODEL_NAME, "")},
        )
        # 时延问题，暂时不支持枚举值修复
        ...
        if self.intent_config.enable_knowledge:
            if not self.api_id:
                ...
                try:
                    ...
                    knowledge_log = self.language_config.get("knowledge_log")
                    logger.warning(f"{knowledge_log} {type(e).__name__}")
    ...
    def invoke(self, inputs: Input, **kwargs) -> Output:
        ...
        logger.info(f"global_intents: {global_intents}")
        global_intent_map = {}
        intent_class = self.language_config.get("classification_default_id")
        # 处理意图检测输入
        try:
            ...
            self._raise_input_error(str(e))
        # 进行 faq 匹配
        if self.intent_config.enable_knowledge:
            try:
                intent_class = self.get_faq_result(
        ...
        if (
            ...
            logger.info("********use faq model.")
            performance_logger.debug(
                f"{self.conversation_id}|intent-faq|{get_intent_duration}"
            )
        ...
        # 获取大模型结果
        llm_output = self.get_llm_result(current_inputs, kwargs.get("span"))
        # 后处理意图检测结果
        intent_res = self._handle_detection_result(llm_output, global_intent_map)
        get_intent_end_time = time.perf_counter()
        get_intent_duration = round((get_intent_end_time - start_time) * 1000)
        logger.info("********use llm model.")
        performance_logger.debug(
            f"{self.conversation_id}|intent-llm|{get_intent_duration}"
        )
        return intent_res
    ...
    async def ainvoke(self, inputs: Input, **kwargs) -> Output:
        ...
        if self.intent_config.enable_knowledge:
            try:
                intent_class = await self.async_get_faq_result(
                    current_inputs, chat_history, **kwargs
                )
            except Exception as e:
                raise JiuWenBaseException(
        ...
        # 获取大模型结果
        llm_output = await self.async_get_llm_result(current_inputs, kwargs.get("span"))
        # 后处理意图检测结果
        intent_res = self._handle_detection_result(llm_output, global_intent_map)
        get_intent_end_time = time.perf_counter()
        get_intent_duration = round((get_intent_end_time - start_time) * 1000)
    ...
    async def async_get_faq_result(self, current_inputs, chat_history, **kwargs):
        """获取faq结果"""
        few_shot_example = ""
        intent_class = self.language_config.get("classification_default_id")
        user_query = self.build_kg_query(current_inputs, chat_history)
        qq_result = await self.async_get_search_answer(user_query, **kwargs)
        if self.kg_conf and self.kg_conf.scope == "faq":
            ...
            for temp in qq_result_ans:
                temp_category = self.language_config.get("classification_default_id")
                for i in range(0, len(self.intent_config.category_name_list)):
                    if temp["content"] == self.intent_config.category_name_list[i]:
                ...
                if (
                    ...
                    and temp_category
                    != self.language_config.get("classification_default_id")
                ):
            ...
            few_shot_example = self.get_few_shot_ex(
                ...
                deepcopy(self.intent_config.category_name_list),
            )
        ...
        current_inputs.update({"few_shot": few_shot_example})
        self.few_shot_example += "\n" + few_shot_example
        return intent_class

    async def async_get_search_answer(self, user_query, **kwargs):
        """获取faq搜索结果"""
        ...
        try:
            ...
            logger.info("search_result" + str(res))
            if res.get("errCode") == StatusCode.SUCCESS.code:
                data = res.get("data")
        except Exception as e:
            logger.info(f"search api error: {str(e)}\n{traceback.format_exc()}")
        prepare_params_end_time = time.perf_counter()
        prepare_params_duration = round((prepare_params_end_time - start_time) * 1000)
        performance_logger.debug(
            f"{self.conversation_id}|onlysearch|{prepare_params_duration}"
        )
        return data

    async def async_get_llm_result(self, current_inputs, span_input):
        """获取llm"""
        llm_inputs = self._pre_process(current_inputs)
        memory_enable = self.mem_conf.get(ENABLE_KEY)
        if isinstance(memory_enable, bool) and memory_enable:
        ...
        logger.info(f"intent detection llm_inputs: {llm_inputs}")
        ...
        try:
            ...
            current_inputs.update(
                {"llm_info": {LLM_INPUTS: llm_inputs, LLM_OUTPUTS: llm_output}}
            )
            ...
            self._raise_llm_invoke_error(str(e))

    def _wrap_intent_class(self, intent_res: str) -> str:
        """封装 意图节点 返回内容，无论中英文，统一使用 "分类x" """
        fixed_prefix = "分类"
        return intent_res.replace(
            self.language_config.get("classification_keywords", "Category "),
            fixed_prefix,
        )

    def get_faq_result(self, current_inputs, chat_history, **kwargs):
        """获取faq结果"""
        few_shot_example = ""
        intent_class = self.language_config.get("classification_default_id")
        user_query = self.build_kg_query(current_inputs, chat_history)
        qq_result = self.get_search_answer(user_query, **kwargs)
        if self.kg_conf and self.kg_conf.scope == "faq":
            qq_result_ans = qq_result.get("output_list", [])
            qq_threshold = float("-inf")
            for temp in qq_result_ans:
    ...
    def get_search_answer(self, user_query, **kwargs):
        """获取faq搜索结果"""
        all_headers = kwargs.get("runtime_auth_headers", {})
        cur_headers = all_headers.get(str(self.api_id)) or all_headers.get(
            "default", {}
        ...
        data = {}
        try:
            res = self.api.invoke(inputs, runtime_auth={"headers": cur_headers})
            logger.info("search_result" + str(res))
            if res.get("errCode") == StatusCode.SUCCESS.code:
                data = res.get("data")
        except Exception as e:
    ...
    def anayls_search(self, search_data):
        ...
        try:
            ...
            for item in doc_list:
                ...
                if item["score"] > self.intent_config.recall_threshold:
                    res.append(addlist)
        except Exception as e:
            logger.info(f"search analysis error: {str(e)}\n{traceback.format_exc()}")
        return res

    def get_few_shot_ex(self, qqresult, intention_list):
        """构造few-shot示例"""
        res = ""
        cnt = 0
        origin_str = self.language_config.get("few_shot_origin_str")
        for temp in qqresult:
            cnt += 1
            # 如果qq匹配返回结果在意图列表里
            if temp.get("content") in intention_list:
                if "title" in temp:
                    addstr = origin_str.format(cnt, temp["title"], temp["content"])
                else:
                    addstr = origin_str.format(
                        cnt, temp["document_name"], temp["content"]
                    )
                res += addstr
        # 如果res里有内容
        if res:
            res = self.language_config.get("few_shot_res_str") + res
        return res
    ...
    def doc_search(self, qqresult):
        ...
        try:
            ...
            res = str(qqresult)
            logger.info(
                f"search analysis doc wrong: {str(e)}\n{traceback.format_exc()}"
            )
        return res

    def get_llm_result(self, current_inputs, span_input):
        """获取llm"""
        llm_inputs = self._pre_process(current_inputs)
        logger.info(f"intent detection llm_inputs: {llm_inputs}")
        # 获取LLM输出并处理
        try:
            llm_output = self.llm.invoke(llm_inputs).content
            current_inputs.update(
                {"llm_info": {LLM_INPUTS: llm_inputs, LLM_OUTPUTS: llm_output}}
            )
            process_on_invoke_info(
    ...
    def add_global_intents(self, global_intents, global_intent_map):
        ...
        logger.info(
            f"category_info: {category_info} and global_intents: {global_intents}"
        )
        # 意图识别节点是否配置有其他意图
        if global_intents:
            category_info_list = []
            ...
            other_index = next(
                (
                    ...
                    if self.language_config.get("other_key_words") in category
                ),
                -1,
            )
            ...
            if self.intent_config.overridable and other_in_local_detection:
                for intent in global_intents:
                    if (
                        hasattr(intent, "name")
                        and self.language_config.get("other_key_words") in intent.name
                    ):
                        # 若配置overridable, 用全局意图的其他意图覆盖子工作流中的其他意图
                        if hasattr(intent, "description") and intent.description:
                            category_key_word = self.language_config.get(
                                "classification_keywords"
                            )
                            category_name = f"{category_key_word}{other_index}"
            ...
            cur_index = (
                ...
                if self.language_config.get("classification_default_id")
                in category_info
            ...
            for intent in global_intents:
                if hasattr(intent, "description") and intent.description:
                    ...
                    category_name = f"{category_key_word}{cur_index + 1}"
    ...
    def intent_detection_post_process(self, result):
        ...
        try:
            ...
            if not isinstance(parsed_dict, dict):
                return self.intent_config.default_class, self.language_config.get(
                    "json_parse_fail_reason"
        ...
        # post_process class informati
        if parsed_dict.get(CLASS) is None and parsed_dict.get(RESULT) is None:
            return self.intent_config.default_class, self.language_config.get(
                "class_key_missing_reason"
            ).format(result=parsed_dict)

        intent_class = parsed_dict.get(CLASS)
        if not intent_class:
            intent_class = (
                str(parsed_dict.get(RESULT))
                .replace("\n", "")
                .replace(" ", "")
                .replace("'", "")
                .replace('"', "")
            )
            category_key_word = self.language_config.get("classification_keywords")
            intent_class = f"{category_key_word}{intent_class}"
        else:
            intent_class = (
                str(intent_class)
                .replace("\n", "")
        ...
        category_keyword = self.language_config.get("classification_keywords")
        match = re.search(rf"{category_keyword}\d+", intent_class)
        if match:
            parsed_dict.update({CLASS: match.group(0)})
        else:  # 如果没找到分类x, 返回默认分类
            return self.intent_config.default_class, self.language_config.get(
                "class_key_missing_reason"
            ).format(result=parsed_dict)
    ...
    def get_config_info(self, cur_conf: dict):
        """
        Get configuration information for user prompt and category information
        Args:
            cur_conf: Original configuration information
        return:
            user_prompt: system prompt
            category_info: category information
            category_list: list of categories
            default_class: default output class
            intent_detection_template: prompt template
            enableHistory: history switch
            enableInput: input switch
            chatHistoryMaxTurn: max rounds of chat history
        """
        ...
        try:
            for branch in cur_conf.get("branches"):
                ...
                if match:
                    ...
                    category_keyword = self.language_config.get(
                        "classification_keywords"
                    )
                    category_list.append(f"{category_keyword}{branch_index}")
                    category_info_list.append(
                        f"{category_keyword}{branch_index}: {branch.get('catalog')}"
                    )
            category_info = "\n".join(category_info_list)
        ...
        if self.language_config.get("classification_default_id") in category_list:
            formatted_config[DEFAULT_CLASS] = self.language_config.get(
    ...
    def _get_config_info(self, cur_conf: dict):
        ...
        # system prompt provided by user
        formatted_config[USER_PROMPT] = cur_conf.get("prompt", "")
        if self.language_config.get("default_int") in category_name_list:
            default_idx = category_name_list.index(
                self.language_config.get("default_int")
            )
            formatted_config[DEFAULT_CLASS] = self.language_config.get(
                "classification_keywords"
            ) + str(default_idx)
    ...
    def _prepare_detection_inputs(self, inputs, chat_history, global_intents):
        ...
        # 如果有user prompt，最后拼接一个换行符
        user_prompt_value = ""
        if self.intent_config.user_prompt:
            user_prompt_value = self.intent_config.user_prompt + "\n"
        ...
        if self.intent_config.enable_history:
            ...
            # 如果chat_history_str有内容，在前面拼
            if chat_history_str:
                chat_history_str = (
                    self.language_config.get("chat_history_keywords") + chat_history_str
                )
    ...
    def _format_chat_history(self, chat_history):
        ...
        for history in filtered_chat_history[
            -self.intent_config.chat_history_max_turn - 1 : -1
        ]:
            chat_history_str += "{}：{}\n".format(
                self.language_config.get("role_map").get(
                    history.get(ROLE, CONTENT),
                    self.language_config.get("user_key_words"),
                ),
                history.get(CONTENT),
            )
    ...
    def _handle_detection_result(self, llm_output, global_intent_map):
        ...
        logger.info("intent_class: {}".format(intent_class))
        ...
        if not self.output_validation(intent_class):
            return dict(
                ...
                reason=self.language_config.get("validation_fail_reason").format(
    ...
    def _handle_global_other_intent(self):
        """处理全局意图并触发中断"""
        result = dict(intent_id="0", reason=self.language_config.get("other_key_words"))
        # 触发全局意图的中断处理
        interrupt_message = {
            "type": MessageSubType.GLOBAL_INTENT.value,
            "intent": result,
    ...
    def _get_intent_id_name(self, intent_config, intent_class):
        intent_res = {
            CLASSIFICATION_ID: self.language_config.get("classification_default_id"),
            CLASSIFICATION_NAME: self.language_config.get(
        ...
        idlist = -1
        fixed_prefix = "分类"
        fixed = self.language_config.get("classification_keywords", fixed_prefix)
        for i, category in enumerate(intent_config.category_list):
            if category == intent_class:
                idx = int(category.replace(fixed, ""))
                idlist = i
        if idx > -1:
            intent_res = {
