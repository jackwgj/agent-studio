...
class RestFulAPI(Invokable, ABC):
    ...
    async def ainvoke(self, inputs: Input, **kwargs) -> Output:
        ...
        try:
            ...
            request_params = await self.request_params_creator.create(
                inputs, file_params_list_type=False, **kwargs
            )
            self._validate_request_params(request_params, **kwargs)
            ...
            logger.error(
                "restful api invoke exception.", exc_info=True
            )
    ...
    async def astream(self, inputs: Input, **kwargs) -> AsyncIterator[Output]:
        ...
        try:
            ...
            trace_manager.on_plugin_error(error)
            logger.error(
                "restful api astream exception.", exc_info=True
            )
    ...
    async def _send_async_post_request(self, request_params):
        ...
        try:
            ...
            logger.error(
                "restful api send_async_post_request exception.", exc_info=True
            )
            return self._format_output(
                -1,
    ...
    def _format_response(self, response, param_wrapper):
        if not self._check_status_ok(response.status_code):
            ...
            raise exception.PluginCommonException(
                code=StatusCode.PLUGIN_RESPONSE_HTTP_CODE_ERROR,
                message=f"plugin response error：{response.text}",
            )
        content = b""
        ...
        try:
            ...
            logger.error(
                f"restful api format_response exception. content: {content.decode('utf-8')}"
            )
    ...
    async def _format_async_response(
        ...
        try:
            ...
            logger.error(
                f"restful api format_async_response exception. content: {content.decode('utf-8')}"
            )
    ...
    def _validate_auth_hook_function(self, request_params: RequestParams, **kwargs):
        ...
        try:
            ...
            if error_code is not None:
                raise JiuWenBaseException(
                    code=error_code, message=getattr(e, "message")
                )
