...
class McpAPI(Invokable, ABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # @override(jiuwen) SSL verification disabled by default (upstream enables it)
    ...
    async def ainvoke(self, inputs: Input, **kwargs) -> Output:
        ...
        try:
            ...
            if isinstance(self.params, list):
                # 提取所有需要剔除的 Header 参数名集合
                header_keys = {p.name for p in self.params if p.method == "Headers"}
                # 过滤 inputs，剔除在 header_keys 中的项
                inputs = {k: v for k, v in inputs.items() if k not in header_keys}
            ...
            # @override(jiuwen) Added URL logging for upstream logic debugging
...
class McpServer(McpAPI, ABC):
    """
    MCP Server class
    This class overrides the upstream implementation. The original does not handle auth info,
    so this inherits McpAPI and reuses related logic.
    """
