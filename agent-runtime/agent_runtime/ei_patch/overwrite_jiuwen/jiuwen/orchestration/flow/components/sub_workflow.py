...
# @override(jiuwen) Fix: message events not returning workflow name
import asyncio
import os
import re
...
class SubWorkflow(Invokable, InteractiveComponent):
    ...
    async def ainvoke(self, inputs: dict[str, Any], **kwargs):
        ...
        params = dict(
            ...
            repo_id=self._runtime_context.get(REPO_ID),
        )
    ...
    async def _process_stream_result(
        ...
        async for item in result:
            ...
            # @override(jiuwen) added workflow_id/name to message events
            if item.code in [
                ...
                if "workflow_name" not in item.data:
                    item.data.update(
                        dict(workflow_name=self._workflow_instance.get_workflow_name())
                    )
            # @override end
            ...
            if item.code in [StreamCode.PARTIAL_CONTENT.value]:
                final_res = final_res + str(item.data.get("answer"))
