...
class LLMChain(Invokable):
    ...
    def _get_streaming_generator_non_stream_mode(self, output, execution_id: str):
        ...
        extra_args = (
            {}
            if output.usage_metadata is None
            else {
                "firstTokenTime": output.usage_metadata.first_token_time,
                "requestStartTime": output.usage_metadata.requests_start_time,
                "deploymentId": output.usage_metadata.task_id,
            }
        )
        yield StreamData(
            ...
            data=get_data_of_streaming_with_metadata(
                answer={USER_FIELDS: formatted_res, **extra_args},
                metadata=self.metadata,
            ),
            execution_id=execution_id,
        )
    ...
    def stream(self, inputs: dict[str, Any], **kwargs):
        """LLM组件流式蹦字调用接口"""
        span = kwargs.get(_SPAN)
        execution_id = span.trace_id if span else ""
        # 大模型节点新增stream开关指定流式或非流式执行
        if self._get_response_format().get("type") == "json":
            ...
            # 增加token消耗信息，非流式场景不需要firstTokenTime
            self.__append_usage_metadata(llm_output, final_output)
        elif self._conf.get("stream") is False:
            inputs = inputs.get(USER_FIELDS)
            self._process_inputs(inputs)
            language_model_inputs = self._get_model_input(inputs=inputs)
            result = self.llm.invoke(language_model_inputs)
            generator_res = self._get_streaming_generator_non_stream_mode(
                output=result, execution_id=execution_id
            )
    ...
    async def astream(self, inputs: dict[str, Any], **kwargs):
        ...
        # 大模型节点新增stream开关指定流式或非流式执行
            ...
            result = await self.llm.ainvoke(language_model_inputs)
            generator_res = self._get_streaming_generator_non_stream_mode(
                output=result, execution_id=execution_id
            )
            output_definition_list = self._get_outputs_list_from_conf()
    ...
    # @override(jiuwen) overridden method
    def _get_model_input(self, inputs: dict) -> LanguageModelInput:
        """
        获取模型输入
        """
        # 解析系统人设以及用户会话模板，构建模型输入参数
        system_template, user_template = self._get_template_content()
        system_prompt_instance = self._get_prompt_instance_by_template(system_template)
        user_prompt_instance = self._get_prompt_instance_by_template(user_template)
        try:
            self._validate_prompt_instance(user_prompt_instance)
            user_prompt = user_prompt_instance.invoke(inputs)
            system_prompt = system_prompt_instance.invoke(inputs)
        ...
        final_prompt = self._get_vision(final_prompt, inputs)
        if system_prompt:
            final_prompt.insert(0, {"role": "system", "content": system_prompt})
    ...
    def _get_history(self, user_prompt: str):
        ...
        # 配置历史大小
        history_size = self._conf.get("historySize")
        if (
            history_size is not None
            and isinstance(history_size, int)
            and history_size >= 0
        ):
            chat_history = self._get_chat_history_pairs(chat_history, history_size)
        origin_history = (
            [chat_history[-1]]
            if chat_history
    ...
    # @override(jiuwen) added method: _get_vision
    # 多模态理解大模型视觉输入填充模型输入,考虑到历史会话将视觉输入放在最后
    def _get_vision(self, final_prompt: list, inputs: dict):
        modelConf = self._conf.get("model", {})
        extension = modelConf.get("extension", {})
        vl_enable = extension.get("vl_enable")
        if vl_enable:
            # 构造视觉输入请求体
            latest_message = final_prompt[-1]
            content = [{"type": "text", "text": latest_message["content"]}]
            ...
            final_prompt[-1] = latest_message
        return final_prompt

    @staticmethod
    def _get_chat_history_pairs(messages: list, history_size: int) -> list:
        """
        截取消息列表中最后size+1个用户提问及其对应的回答
        :param messages: 消息列表，格式为[{"role": "user"/"assistant", "content": "xxx"}, ...]
        :param history_size: 要获取的用户提问数量（最终返回history_size+1个用户提问，包括最后一个）
        :return: 截取后的消息列表
        """
        if not messages:
            return []
        while messages and messages[-1].get("role", "user") != "user":
            messages = messages[:-1]
        if not messages:
            return []

        user_indices = [
            i for i, msg in enumerate(messages) if msg.get("role", "user") == "user"
        ]
        if len(user_indices) <= history_size:
            return messages.copy()

        # 获取最后history_size+1个用户消息的索引
        last_user_indices = user_indices[-(history_size + 1) :]
        start_index = last_user_indices[0]
    ...
    # @override(jiuwen) overridden method
    def _get_template_content(self) -> tuple[str, str]:
        template_content_list = self._conf.get("templateContent") or []
        system_prompt = [
            element
            for element in template_content_list
            if element.get(_ROLE) == ChatHistoryRole.SYSTEM.value
        ]
        user_prompt = [
            element
            for element in template_content_list
        ...
        return str(system_prompt[0].get("content")) if len(
            system_prompt
        ) != 0 else "", str(user_prompt[0].get("content"))
...
# @override(jiuwen) added method: coroutine2async_generator
async def coroutine2async_generator(coro):
    """将协程转换为异步迭代器
    协程的返回结果为同步迭代器
    """
    async_generator = await coro
