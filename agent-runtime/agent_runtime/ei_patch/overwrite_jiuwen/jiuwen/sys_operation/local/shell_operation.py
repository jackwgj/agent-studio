...
from jiuwen.sys_operation.base import OperationMode
from jiuwen.sys_operation.local.utils import (
    OperationUtils,
    StreamEvent,
    StreamEventType,
)
from jiuwen.sys_operation.registry import operation
from jiuwen.sys_operation.result.base_result import build_operation_error_result
from jiuwen.sys_operation.result.shell_operation_result import (
    ExecuteCmdResult,
    ExecuteCmdChunkData,
    ExecuteCmdData,
    ExecuteCmdStreamResult,
)
from jiuwen.sys_operation.shell import BaseShellOperation
...
class ShellOperation(BaseShellOperation):
    ...
    async def execute_cmd(
        ...
        *,
        ...
        def _create_exec_cmd_err(
            error_msg: str, data: Optional[ExecuteCmdData] = None
        ) -> ExecuteCmdResult:
            ...
            logger.error("Failed to execute cmd", extra={"error": error_msg})
            return err_result
        ...
        try:
            ...
            process_handler = OperationUtils.create_handler(
                process=proc, encoding=encoding, timeout=timeout
            )
            ...
            success_result = ExecuteCmdResult(
                code=StatusCode.SUCCESS.value[0],
                message=StatusCode.SUCCESS.value[1],
                data=ExecuteCmdData(
                    command=command,
                    cwd=str(actual_cwd),
                    exit_code=invoke_data.exit_code,
    ...
    async def execute_cmd_stream(
        ...
        *,
        ...
        def _create_exec_cmd_stream_err(
            error_msg: str, data: Optional[ExecuteCmdChunkData] = None
        ) -> ExecuteCmdStreamResult:
        ...
        try:
            ...
            def _stream_event_trans(
                stream_event_data: StreamEvent, data_idx: int
            ) -> Optional[ExecuteCmdStreamResult]:
                """Transform raw StreamEvent to structured ExecuteCmdStreamResult"""

                def _handle_std_out_err(
                    event: StreamEvent, idx: int
                ) -> ExecuteCmdStreamResult:
                    """Handle STDOUT/STDERR stream events"""
                    chunk_data = ExecuteCmdChunkData(
                        text=event.data, type=event.type.value, chunk_index=idx
                    )
                ...
                def _handle_process_exit(
                    ...
                    chunk_data = ExecuteCmdChunkData(
                        chunk_index=idx, exit_code=exit_code
                    )
    ...
    def _check_allowlist(self, command: str) -> bool:
        ...
        if (
            not hasattr(self._run_config, "shell_allowlist")
            or self._run_config.shell_allowlist is None
        ):
            return True

        cmd_prefix = command.split()[0] if command.strip() else ""
        return any(
            cmd_prefix == allowed or cmd_prefix.endswith(os.sep + allowed)
