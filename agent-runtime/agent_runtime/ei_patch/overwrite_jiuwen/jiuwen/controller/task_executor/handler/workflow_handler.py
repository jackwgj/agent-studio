...
# @override(jiuwen) Fix: controller message missing workflow name; tool call failure and -1 error code
import json
import os
import time
...
class WorkflowHandler(BaseHandler):
    ...
    ) -> SpiffWorkflowControllerWorkflow:
        """创建工作流实例"""
    ...
    def _check_questioner_interrupt(workflow, func_res: StreamData) -> None:
        ...
        if (
            ...
            if hasattr(workflow, "graph_engine") and hasattr(
                workflow.graph_engine, "graph_instance"
            ):
                workflow.graph_engine.graph_instance.stop_bpmn_flag = True
    ...
    async def stream_execute_workflow_for_intent_detection(
        ...
        try:
            ...
            logger.error(
                f"Failed to execute intent detection workflow {workflow_context.workflow_name}: {e}",
                simple_log=f"Failed to execute intent detection workflow"
                f"{workflow_context.workflow_name}: {type(e)}",
            )
            raise JiuWenBaseException(
                error_code=JiuWenBaseExceptionStatusCode.CONTROLLER_INTENT_WORKFLOW_EXECUTION_ERROR.code,
                message=format_exception_reason(e),
            ) from e
        finally:
    ...
    async def _handle_user_interact_state(
        ...
        self.context_manager.update_workflow_status(
            workflow_id, workflow_context.type, WorkflowStatus.INTERRUPTED
        )
        self.context_manager.set_workflow_state(
            workflow_id,
            workflow_context.type,
            workflow_instance.graph_engine.graph_instance.get_state(),
        )
    ...
    async def _stream_handle_workflow_from_function_call(
        ...
        async def _tap_exec_result(gen: AsyncGenerator):
            nonlocal questioner_interrupted
        ...
        try:
            exec_result, exec_latency = await self.execute_workflow_from_function_call(
                workflow_instance, function_call, **runtime_data
            )
            ...
            if self._check_user_interact(workflow_instance, questioner_interrupted):
                ...
                yield MessageConverter.create_workflow_completion_message(
                    workflow_context=workflow_context, exec_res=exec_result
                )
        finally:
            await workflow_instance.graph_engine.graph_instance.async_clean_up()
    ...
    async def _stream_execute_workflow(
        ...
        # 创建工作流实例
        workflow_instance = await self.create_workflow_instance(
            workflow_context, task.input_data.get("conversation_id", ""), user_id
        )
        try:
        ...
        if isinstance(result, Message):
            ...
            async for output in result:
                ...
                if output.data:
                    ...
                    # @override(jiuwen) added workflow_name to output
    ...
    def _update_output_global_variables_for_request(
        ...
        runtime_context = workflow_instance.graph_engine.graph_instance.runtime_context
    ...
    def _merge_workflow_arguments(
        ...
        try:
            # @override(jiuwen) arguments may not exist
            current_args = json.loads(function_call.get("arguments", "{}"))
            # @override end
