...
class ContextManager:
    ...
    def set_memory_request_params(
        self, memory_repo_id: str, enable_memory_retrieve: bool
    ):
        """set request params of memory"""
        self.memory_repo_id = memory_repo_id
        self.memory_enable = self.memory_enable and enable_memory_retrieve
    ...
    async def get_memory_message(self, query: str) -> HumanMessage | None:
        ...
        if (
            user_id == ""
            or self.memory_repo_id == ""
            or not (isinstance(query, str) and query.strip())
        ):
            return None
        self._memory_message = await self._memory_proxy.get_related_memory_msg(
            user_id=user_id, scope_id=self.memory_repo_id, query=query
        )
        return self._memory_message
    ...
    def set_intent_msg(self, result: dict):
        self._intent_messages = result

    def get_intent_msg(self):
        msg = self._intent_messages
        self._intent_messages = None
        return msg
