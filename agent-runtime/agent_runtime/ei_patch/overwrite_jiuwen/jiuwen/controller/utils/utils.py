...
class WorkspaceUtils:
    ...
    async def gen_tools(
        ...
        mcps = await WorkspaceUtils.get_all_mcp_apis(mcps, **kwargs)
    ...
    @staticmethod
    async def get_all_mcp_apis(mcps, **kwargs) -> list:
        """获取所有MCP服务器的API
        ...
        mcp_apis = []
        for mcp in mcps:
            mcp_tools = await mcp.ainvoke(inputs={}, **kwargs)
            if mcp_tools:
                for mcp_tool in mcp_tools.tools:
                    mcp_apis.append(
                        McpAPI(
                            ...
                            auth=mcp.auth,
                            params=mcp.params,
                            input_parameters=mcp.input_parameters,
                            **kwargs,
                        )
                    )
        return mcp_apis
