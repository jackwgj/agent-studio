...
# @override(jiuwen) Fix: missing call-chain data in query results
"""
utils
"""
...
from jiuwen.insight.handlers.data import InvokeData, InvokeType
...
def format_invoke_data(invoke: InvokeData) -> Dict:
    ...
    try:
        ...
        # @override(jiuwen) added plugin invoke attributes handling
        if invoke.invoke_type == InvokeType.PLUGIN:
            attributes = (invoke.serialized or {}).get("instance_attributes")
            invoke_dict["serialized"] = {
                "instance_attributes": {
                    "plugin_name": attributes.get("plugin_name", ""),
                    "is_mcp": attributes.get("is_mcp", False),
                    "server_name": attributes.get("server_name", ""),
                }
            }
        # @override end
