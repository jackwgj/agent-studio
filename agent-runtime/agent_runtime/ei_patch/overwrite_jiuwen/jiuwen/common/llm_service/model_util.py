...
class ModelUtil:
    ...
    def switch_message(cls, messages: list):
        ...
        for mess in messages:
            if mess.get(ROLE_STR) == "system":
                ...
                tmp_tool_call_id = ""
                if cur_tool_call_id:
                    tmp_tool_call_id = cur_tool_call_id
                    cur_tool_call_id = ""
                request_mess.append(
                    FunctionMessage(
                        content=mess.get(content_str),
                        name=mess.get("name", ""),
                        tool_call_id=tmp_tool_call_id,
                    )
                )
                ...
                cur_tool_call_id = cls.switch_assistant_msg(
                    content_str, cur_tool_call_id, mess, request_mess
                )
        return request_mess

    @classmethod
    def switch_assistant_msg(cls, content_str, cur_tool_call_id, mess, request_mess):
    ...
    def convert_message_to_prompt(
        ...
        if not functions_dict:
            system_prompt = TemplateManager().get(
                name=cls.system_message_prompt_without_tool,
                filters={"model_name": model_name},
            )
        else:
            system_prompt = TemplateManager().get(
                name=cls.system_message_prompt, filters={"model_name": model_name}
            )
    ...
    def truncate_params(cls, payload: Dict[str, Any]):
        """Truncate temperature and top_p parameters between [0.0, 1.0].
