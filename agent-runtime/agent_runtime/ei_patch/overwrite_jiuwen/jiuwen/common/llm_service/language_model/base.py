...
class BaseChatModel(Invokable, Runnable[LanguageModelInput, LanguageModelOutput], ABC):
    ...
    def _convert_message_to_dict(message: BaseMessage) -> Dict[str, Any]:
        ...
        if isinstance(message, HumanMessage):
            ...
            if isinstance(message.tool_calls, List) and len(message.tool_calls) > 0:
                ...
                message_dict.update(dict(function_call=function_call_list))
            elif isinstance(message.tool_calls, ToolCall):
                message_dict.update(
                    dict(
                        function_call=dict(
    ...
    @staticmethod
    def _convert_message_to_requests_dict(message: BaseMessage) -> Dict[str, Any]:
        """Convert a message to a dictionary.
        ...
        """
        role_str = "role"
        reasoning_content_str = "reasoning_content"
        content_str = "content"
        function_str = "tool"
        if isinstance(message, HumanMessage):
            message_dict = {role_str: "user", content_str: message.content}
            ...
            message_dict = {
                ...
                reasoning_content_str: message.reasoning_content
    ...
    async def astream(
        ...
        if isinstance(inputs, dict):
            return self._astream(inputs.get("messages"), inputs.get("tools"), **kwargs)
        return self._astream(self.__convert_input_to_message(inputs=inputs))
