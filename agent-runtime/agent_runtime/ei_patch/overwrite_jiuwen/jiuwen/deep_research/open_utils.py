...
from jiuwen.serve.controllers.execution.open_utils import EICacheUtils
from yarl import URL

MAX_TEMPLATE_CACHE_NUM = int(os.getenv(MAX_TEMPLATE_CACHE_NUM_KEY, str(100)))
HTTP_STATUS_CODE = 300
cache_template_queue = EICacheUtils(
    capacity=MAX_TEMPLATE_CACHE_NUM, should_serialize=True, cache_name="template"
)
...
async def async_template_load(path: str, is_external_path: bool) -> str:
    ...
    if is_external_path:
        ...
        async with aiohttp.ClientSession(
            ...
            async with session.get(url=URL(path, encoded=True)) as res:
