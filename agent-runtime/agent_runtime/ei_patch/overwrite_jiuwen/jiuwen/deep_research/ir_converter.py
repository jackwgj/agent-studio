...
from jiuwen.deep_research.adapter_open_jiuwen_ds import DeepResearchConfigAdapter
...
class DSIRConverter:
    ...
    async def create_deepresearch_agent(
        ...
        # 将原有的商用DS Config 转换为 OpenJiuwen DS Config
        open_jiuwen_ds_config = DeepResearchConfigAdapter.adapt(asdict(agent_config))
        # 传入工厂
        deepresearch_agent = agent_factory.create_agent(open_jiuwen_ds_config)
        return deepresearch_agent, open_jiuwen_ds_config, template
    ...
    async def ir_to_deepresearch(
        ...
        (
            ...
            ready_to_use_config,
            template,
        ) = await DSIRConverter.create_deepresearch_agent(
            deepresearch_agent_config, req
        )
        return agent, ready_to_use_config, template
