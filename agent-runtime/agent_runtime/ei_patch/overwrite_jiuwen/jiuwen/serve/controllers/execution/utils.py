...
# @override(jiuwen) pre-optimization import: from queue import Queue, Empty
from asyncio import Queue, QueueEmpty  # @override(jiuwen) post-optimization import
...
# @override(jiuwen) pre-optimization import: from jiuwen.insight.client import Client
# @override(jiuwen) pre-optimization import: from jiuwen.insight.handlers.remote import InsightHandler
from jiuwen.insight.async_client import (
    Client,
)  # @override(jiuwen) post-optimization import
from jiuwen.insight.handlers.async_handler import (
    InsightHandler,
)  # @override(jiuwen) post-optimization import
...
from jiuwen.orchestration.flow.workflow import Workflow, build_workflow
...
_AGENT_ERROR_REPLY = "failed to generate response"
_UTF_8 = "utf-8"
...
item_code_to_conversation_event_type = {
    ...
    StreamCode.INTENT_MSG.value: ConversationEvent.INTENT_MSG,
}
...
async def distribute_execution_request(
    ...
    if execution_data.instance_type == IRType.MultiAgents:
        try:
            ...
            await AsyncStateManager().delete_state(req.conversation_id)
            logger.error(f"execute_agent_group error: {e}")
            raise
    ...
    if execution_data.instance_type == IRType.DeepResearch:
        try:
            ...
            logger.error(f"execute_deepresearch error: {e}")
            raise
    if execution_data.instance_type == IRType.DeepResearch:
        try:
            return await execute_deepresearch(req, execution_data)
...
async def _execute_agent(req: ExecutionRequest, execution_data: ExecutionData):
    ...
    if sys_op_card_data is not None:
        from jiuwen.sys_operation import SysOperationCard
    ...
    if req.response_mode == ResponseMode.STREAMING:
        ...
        if agent.get_control_mode() == "ReAct":
        ...
        elif agent.get_control_mode() == "ToolUse":
            resp = _format_agent_streaming_output(
                origin_output=streaming_output,
                insight_queue=insight_client.data_queue,
                task_id=agent.task_id,
        ...
        # @override(jiuwen) added performance logging
        end_time = time.perf_counter()
        duration = round((end_time - start_time) * 1000)
        performance_logger.info(f"execute_agent|{duration}")
        # @override end
        return StreamingResponse(resp, media_type="text/event-stream")
...
async def _execute_workflow(req: ExecutionRequest, execution_data: ExecutionData):
    ...
    """
    wf: Workflow = execution_data.instance
    start_time = time.perf_counter()

    # 流式调用
    if req.response_mode == ResponseMode.STREAMING:
        streaming_output = await wf.astream(
            query=req.query, params=req.params.model_dump()
        )
        end_time = time.perf_counter()
        duration = round((end_time - start_time) * 1000)
    ...
    # 非流式调用
    blocking_output = await wf.ainvoke(query=req.query, params=req.params.model_dump())
...
async def execute_single_component(
    ...
    此异步函数用于处理单个组件的执行请求。它接收一个组件执行请求和执行数据，然后使用这些数据来执行工作流，并返回一个流式响应。

    Args:
        req (ComponentExecutionRequest): 组件执行请求对象，包含执行所需的输入和参数。
        execution_data (ExecutionData): 执行数据对象，包含工作流实例和其他执行相关信息。
    ...
    Notes:
        - 函数首先设置工作流的运行时上下文，将`req.inputs`设置为`single_node_inputs`。
        - 使用`wf.astream`方法异步执行工作流，并获取输出。
        - 最后，通过`_format_component_output`函数格式化输出，并返回一个`StreamingResponse`对象。
        - 函数确保在执行完成后删除工作流实例以释放资源。
    """

    wf: Workflow = execution_data.instance
    wf.runtime_context.set("single_node_inputs", req.inputs)
    output = await wf.astream(
        query=req.inputs.get("query", ""), params=req.params.dict()
    )
...
async def _format_component_output(
    ...
    async for item in origin_output:
        ...
        if item.code == StreamCode.ERROR.value:
            ...
            # @override(jiuwen) diverges from upstream
            item.data.update(
                dict(
                    message=extend_error_template(
                        execution_data.instance,
                        WORKFLOW_UNIFIED_ERROR_INFORMATION_UNSAFE,
                    ).format(code_of_data, item.data.get("message"))
                )
            )
            # @override end
            error_flag = True
...
async def post_process_agent_group_streaming_output(
    ...
    try:
        ...
        logger.error(f"Error in updating or deleting state: {e}")
        raise JiuWenBaseException(
...
def extend_error_template(workflow_instance, template):
    return template
...
async def post_process_workflow_streaming_output(
    ...
    error_flag = False
    start_time = time.perf_counter()
    workflow_id = workflow_instance.get_workflow_id()
    async for item in origin_output:
        if not execution_id:
            execution_id = item.execution_id
        if start_flag:
        ...
        if item.code == StreamCode.ERROR.value:
            ...
            item.data.update(
                dict(
                    message=extend_error_template(
                        workflow_instance, WORKFLOW_UNIFIED_ERROR_INFORMATION_UNSAFE
                    ).format(code_of_data, item.data.get("message"))
                )
            )
            # @override end
        ...
        # @override(jiuwen) Message node returns workflow ID and name
        if item.code in [
            ...
            if "workflow_name" not in item.data:
                item.data.update(dict(workflow_name=workflow_instance.get_workflow_name()))
        # @override end
...
class AgentIrUtils:
    ...
    def update_prompt_plugin(
        ...
        # 设置插件
        AgentIrUtils.update_plugins_from_ir(agent, ir_data)
        # 设置MCP
    ...
    @staticmethod
    def update_plugins_from_ir(agent, ir_data):
        """从IR中更新插件信息"""
        plugin_irs = ir_data.get("configs", {}).get("plugins")
        if plugin_irs:
            current_plugins = [
                PluginIRConverter.ir_to_plugin(ir_data) for ir_data in plugin_irs
            ]
        else:
            current_plugins = []
    ...
    @staticmethod
    async def get_controller_workflow_instance(ir_data: dict):
        """
        Get workflow instances from IR data

        Args:
            ir_data: IR configuration data dictionary

        Returns:
            Tuple containing general workflows, start workflow, end workflow, and global workflows
        """
        configs = ir_data.get("configs", {})

        # Extract general workflows
        general_workflows = asyncio.gather(
            *[
                AgentIrUtils.generate_controller_workflow_instance(workflow_info)
                for workflow_info in configs.get("workflows", [])
            ]
        )

        # Extract start workflow if exists
        start_workflow_info = configs.get("start_workflow")
        start_workflow_instance = None
        if start_workflow_info:
            start_workflow_instance = (
                await AgentIrUtils.generate_controller_workflow_instance(
                    start_workflow_info
                )
            )

        # Extract end workflow if exists
        end_workflow_info = configs.get("end_workflow")
        end_workflow_instance = None
        if end_workflow_info:
            end_workflow_instance = (
                await AgentIrUtils.generate_controller_workflow_instance(
                    end_workflow_info
                )
            )

        # Extract global workflows from global intents
        global_workflows = []
        for intent in configs.get("global_intents", []):
            if intent.get("handler_type") == HandlerType.WORKFLOW.value and intent.get(
                "handler"
            ):
                global_workflow = (
                    await AgentIrUtils.generate_controller_workflow_instance(
                ...
                # Include intent information with the workflow for reference
                global_workflow.intent_id = intent.get("intent_id")
                global_workflow.intent_name = intent.get("name")
                global_workflows.append(global_workflow)
    ...
    @staticmethod
    async def generate_controller_workflow_instance(workflow_info):
        """generate controller workflow instance"""
        ...
        workflow_instance = SpiffWorkflowControllerWorkflow(
            workflow_id=workflow_ir.get("workflowName"),
            description=workflow_ir.get("description"),
            params=workflow_params.get("parameters"),
            graph_instance=await build_workflow(workflow_ir, is_workflow_mode=False),
        )
    ...
    def get_intent_identification_configs(
        ...
        if config.enable_multi_layer_detection:
            ...
            if not multi_layer_id_cfg_data:
                error_msg = "multi_layer_id_cfg cannot be empty!"
                logger.error(error_msg)
                raise JiuWenBaseException(
                    StatusCode.CONTROLLER_INTENT_DETECTION_ERROR.code, message=error_msg
                )
    ...
    @staticmethod
    async def get_controller_workflow_instances_from_state(ir_data: dict, agent_state):
        """Get controller workflow instances including workflows, start/end workflow, and global intent workflows

        Args:
            ir_data: Agent IR data containing workflow configurations
            agent_state: Agent state containing workflow states

        Returns:
            tuple: (workflows, start_workflow_instance, end_workflow_instance, global_intent_workflows)
        """
        configs = ir_data.get("configs", {})

        # 获取未完成的workflow状态
        unfinished_workflows = {
            state.workflow_state.workflow_name: state
            for state in agent_state.workflow_states
        }

        # 1. 处理常规工作流
        workflows = []
        workflow_ir_paths = configs.get("workflows", [])
        for workflow_info in workflow_ir_paths:
            workflow_instance = await AgentIrUtils.create_workflow_instance_from_state(
                workflow_info, unfinished_workflows
            )
        ...
        if start_workflow_info:
            start_workflow_instance = (
                await AgentIrUtils.create_workflow_instance_from_state(
                    start_workflow_info, unfinished_workflows
                )
            )
        ...
        if end_workflow_info:
            end_workflow_instance = (
                await AgentIrUtils.create_workflow_instance_from_state(
                    end_workflow_info, unfinished_workflows
                )
            )
    ...
    @staticmethod
    async def create_workflow_instance_from_state(workflow_info, unfinished_workflows):
        """从workflow_info和未完成的状态创建工作流实例

        Args:
            workflow_info: 包含ir_path的工作流信息字典
            unfinished_workflows: 未完成的工作流状态字典

        Returns:
            SpiffWorkflowControllerWorkflow: 工作流实例，如果创建失败则返回None
        """
        try:
            # 先加载workflow_ir
            workflow_ir = await async_ir_load(workflow_info.get("ir_path"))
            ...
            # 检查是否有未完成的workflow状态
            if workflow_name in unfinished_workflows:
                # 使用未完成的workflow状态创建工作流实例
                graph_instance = await build_workflow(
                    unfinished_workflows[workflow_name], is_workflow_mode=False
                )
                workflow_params = WorkSpace.get_workflow_params(workflow_ir)
            ...
            # 使用generate_controller_workflow_instance创建新的工作流实例
            return AgentIrUtils.generate_controller_workflow_instance(workflow_info)
        except Exception as e:
            # 记录错误但不中断整个流程
            logger.error(
                f"Failed to create workflow instance from {workflow_info.get('ir_path')}: {str(e)}"
            )
            return None
    ...
    def parse_input_variables_from_ir(input_variables):
        ...
        def parse_variable(var):
            ...
            default = var.get("default")  # IR uses 'default_value'
