...
class FlowQA(WorkflowComponent):
    ...
    def _load_state_from_session(session: Session) -> QAState:
        state_dict = None
        if hasattr(session, "get_global_state"):
            state_dict = session.get_global_state("flow_qa_state")
        if state_dict:
            return QAState(
    ...
    def _store_state_to_session(state: QAState, session: Session):
        ...
        if hasattr(session, "update_global_state"):
            session.update_global_state({"flow_qa_state": state_dict})
        ...
        try:
            ...
            if self._conf.need_reply:
                if self._node_state.status == ExecutionStatus.START:
                    ...
                    user_intent = self.get_latest_user_query_intent(session)
                    ...
                    if user_intent and len(user_intent) > 1:
                        ...
                        conv_history = self.get_latest_chat_history(session)
                        user_response = (
