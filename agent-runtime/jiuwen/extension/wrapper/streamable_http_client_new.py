...
"""
新版 Streamable HTTP Client

设计要点：
- __client_name__ = "streamable_http_new" 用于自动注册
...
class StreamableHttpClientNew(McpClient):
    """
    新版 Streamable HTTP Client

    设计要点：
    - __client_name__ = "streamable_http_new" 用于自动注册到 ClientRegistry
    - 每次调用新建 client/session（参考 mcpapi.py 模式）
    - 通过 headers 参数传入认证信息
    - 支持 mcp_choose_tools 参数过滤工具列表
    """

    __client_name__ = "streamable_http_new"
