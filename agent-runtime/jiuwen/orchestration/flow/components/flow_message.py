...
"""
This module contains the FlowMessage class, which is a subclass of Invokable.
FlowMessage is responsible for returning the message in workflow executing process.
"""
...
MESSAGE_RUNTIME_KEYWORD = "message_outputs"
...
class FlowMessageComponentConfig(BaseModel):
    """
    消息组件 配置校验
    """
...
class FlowMessage(Invokable, MessageComponentBase):
    """Message node."""
    ...
    def __init__(
        ...
        if CONFIG_TEMPLATE_KEYWORD not in conf:
            raise JiuWenBaseException(
                message=StatusCode.WORKFLOW_MESSAGE_TEMPLATE_NOT_FOUND_ERROR.errmsg,
                error_code=StatusCode.WORKFLOW_MESSAGE_TEMPLATE_NOT_FOUND_ERROR.code,
            )
        ...
        self.template = Prompt(
            template=Template(
                name="message_template", content=self.origin_template_conf
            )
        )
        if not isinstance(runtime_context.get(MESSAGE_RUNTIME_KEYWORD), list):
            runtime_context.set(MESSAGE_RUNTIME_KEYWORD, [])
        ...
        self.node_type = NodeType.MESSAGE.value
        ...
        val = conf.get("enable_history", True)
        if isinstance(val, bool):
            self.enable_history = val
        elif isinstance(val, str):
            self.enable_history = val.strip().lower() != "false"
        else:
    ...
    def invoke(self, inputs, **kwargs):
        ...
        try:
            ...
            raise JiuWenBaseException(
                message=StatusCode.WORKFLOW_MESSAGE_TEMPLATE_ASSEMBLE_ERROR.errmsg,
                error_code=StatusCode.WORKFLOW_MESSAGE_TEMPLATE_ASSEMBLE_ERROR.code,
            ) from e

        try:
            # a list which refer from global runtime_context
            message_outputs = self.runtime_context.get(MESSAGE_RUNTIME_KEYWORD)
            message_outputs.append(self._format_message_output(output=flow_message))
            return dict(result=flow_message)
        except Exception as e:
            raise JiuWenBaseException(
                message=StatusCode.WORKFLOW_MESSAGE_INVOKE_UNEXPECTED_ERROR.errmsg,
                error_code=StatusCode.WORKFLOW_MESSAGE_INVOKE_UNEXPECTED_ERROR.code,
            ) from e
    ...
    async def ainvoke(self, inputs: dict, **kwargs):
        ...
        try:
            ...
            raise JiuWenBaseException(
                message=StatusCode.WORKFLOW_MESSAGE_AINVOKE_UNEXPECTED_ERROR.errmsg,
                error_code=StatusCode.WORKFLOW_MESSAGE_AINVOKE_UNEXPECTED_ERROR.code,
            ) from e
    ...
    def _format_message_output(self, output):
        return dict(
