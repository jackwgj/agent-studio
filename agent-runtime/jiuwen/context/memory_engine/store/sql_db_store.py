...
from jiuwen.context.memory_engine.store import BaseDbStore
...
class SqlDbStore:
    ...
    async def get(
        self, table: str, table_id: str, columns: list[str] = None
    ) -> dict[str, Any] | None:
        """
        Sql db get
        """
        try:
            ...
            if columns:
                ...
                stmt = select(t)
            stmt = stmt.where(t.c.id == table_id)
            async with self.async_session() as session:
                async with session.begin():
    ...
    async def get_with_sort(
        ...
        """
        Sql db get with sort
        """
        try:
            t = await self._get_table(table)
            if sort_by not in t.c:
                raise ValueError(
                    f"Sort column '{sort_by}' does not exist in table '{table}'"
                )
            clauses = [t.c[col] == val for col, val in filters.items() if col in t.c]
            stmt = select(t)
            if clauses:
    ...
    async def batch_get(
        self, table: str, conditions_list: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        Sql sb batch get
        """
        t = await self._get_table(table)
        clauses = [
            or_(*[t.c[col] == val for col, val in cond.items()])
    ...
    async def condition_get(
        self, table: str, conditions: Dict[str, List[Any]], columns: List[str] = None
    ) -> List[Dict[str, Any]] | None:
        """
        Sql sb condition get
        """
        try:
            t: Table = await self._get_table(table)
            stmt = select(t) if not columns else select(*[t.c[col] for col in columns])
            clause_list = []
    ...
    async def update(self, table: str, conditions: dict, data: dict) -> bool:
        ...
        t = await self._get_table(table)
        clauses = [
            t.c[col].in_(vals) if isinstance(vals, list) else t.c[col] == vals
