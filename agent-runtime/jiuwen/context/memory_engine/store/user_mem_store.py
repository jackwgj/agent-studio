...
class UserMemStore:
    ...
    async def write(
        self, user_id: str, scope_id: str, mem_id: str, data: dict[str, Any]
    ) -> bool:
        """write data to store"""
        if not data:
        ...
        if await self.kv_store.exists(user_mem_key):
            logger.error(
                f"write failed, user memory already exists for user_id={user_id}, scope_id={scope_id}, "
                f"mem_id={mem_id}"
            )
            return False
        ...
        if UserMemStore.MEM_TYPE_FIELD_KEY in data.keys():
            # mem_type ids
            user_mem_ids_key = self.__get_user_ids_key(
                user_id, scope_id, data[UserMemStore.MEM_TYPE_FIELD_KEY]
            )
            user_mem_ids_value = await self.kv_store.get(user_mem_ids_key) or ""
            await self.kv_store.set(
                user_mem_ids_key, self.__write_id(user_mem_ids_value, mem_id)
            )

            # user profile topic ids
            if (
                data[UserMemStore.MEM_TYPE_FIELD_KEY] == MemoryType.USER_PROFILE.value
                and UserMemStore.TOPIC_FIELD_KEY in data.keys()
                and data[UserMemStore.TOPIC_FIELD_KEY] is not None
            ):
                ...
                await self.kv_store.set(
                    user_mem_topic_key, self.__write_id(user_mem_topic_value, mem_id)
                )
    ...
    async def update(
        ...
        if not await self.kv_store.exists(user_mem_key):
            logger.error(
                f"update failed, user memory does not exists for user_id={user_id}, scope_id={scope_id}, "
                f"mem_id={mem_id}"
            )
            return False
    ...
    async def batch_get(
        self, user_id: str, scope_id: str, mem_ids: list[str]
    ) -> list[dict[str, Any]] | None:
        """get data from given ids"""
        keys_list = [
            self.__get_user_mem_key(user_id, scope_id, mem_id) for mem_id in mem_ids
        ]
        value_list = await self.kv_store.mget(keys_list)
        if not value_list:
            return []
        return [json.loads(value) for value in value_list if value is not None]

    async def get_all(
        self, user_id: str, scope_id: str, mem_type: str = None
    ) -> list[dict[str, Any]] | None:
    ...
    async def get_by_topic(
        self, user_id: str, scope_id: str, topic: str
    ) -> list[dict[str, Any]] | None:
    ...
    async def get_in_range(
        self, user_id: str, scope_id: str, start_idx: int, end_idx: int
    ) -> list[dict[str, Any]] | None:
        """
        get data from given user_id|scope_id|start_idx|end_idx
        """
        user_ids_key = self.__get_user_ids_key(user_id, scope_id)
        if not await self.kv_store.exists(user_ids_key):
            return None
    ...
    async def __inner_delete(self, user_id: str, scope_id: str, mem_id: str):
        ...
        if not await self.kv_store.exists(user_mem_key):
            logger.warning(
                f"delete failed, user memory does not exists for user_id={user_id}, scope_id={scope_id}, "
                f"mem_id={mem_id}"
            )
            return
        ...
        if data:
            ...
            if UserMemStore.MEM_TYPE_FIELD_KEY in dict_value:
                user_mem_ids_key = self.__get_user_ids_key(
                    user_id, scope_id, dict_value[UserMemStore.MEM_TYPE_FIELD_KEY]
                )
                ...
                if (
                    ...
                    and UserMemStore.TOPIC_FIELD_KEY in dict_value
                    and dict_value[UserMemStore.TOPIC_FIELD_KEY] is not None
                ):
                    user_mem_topic_key = self.__get_concatenation_key(
                        [
                            user_id,
    ...
    def __delete_id_by_value(self, data_list: str, id_str: str) -> str:
        ...
        for i in range(total):
            ...
            if chunk == id_str:
                return (
                    data_list[: i * self.BYTE_NUM_PER_ID]
                    + data_list[(i + 1) * self.BYTE_NUM_PER_ID :]
                )
        return data_list
    ...
    def __get_ids_in_range(
        self, data_list: str, start_idx: int, end_idx: int
    ) -> list[str]:
        total = len(data_list) // self.BYTE_NUM_PER_ID
