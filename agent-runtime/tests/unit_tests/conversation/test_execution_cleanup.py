...
@pytest.mark.asyncio
async def test_execution_cleanup_never_removes_persistent_output():
...
@pytest.mark.asyncio
async def test_execution_cleanup_rejects_aio_nonzero_result_without_is_ok():
    context = ConversationExecutionContext.create(
        ConversationIdentity("p", "w", "u", "c", "e"), "/workspace"
    )
...
@pytest.mark.asyncio
async def test_execution_cleanup_rejects_nonzero_shell_exit_code():
    context = ConversationExecutionContext.create(
        ConversationIdentity("p", "w", "u", "c", "e"), "/workspace"
    )
    shell = SimpleNamespace(
